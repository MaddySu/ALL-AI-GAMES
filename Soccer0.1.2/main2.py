...
# Import images
imgBackground = cv2.imread("3.png")
imgGameOver = cv2.imread("5.png", cv2.IMREAD_UNCHANGED)
imgBoll = cv2.imread("12-111.png", cv2.IMREAD_UNCHANGED)
imgBat1 = cv2.imread("12-1111.png", cv2.IMREAD_UNCHANGED)
imgBat2 = cv2.imread("12-1111 copy.png", cv2.IMREAD_UNCHANGED)
...
while True:
    ...
    if bollPos[0] < 40 or bollPos[0] > 1880:  # Adjusted positions
        ...
        if misses >= 3:
            ...
            bollPos = [100, 100]  # Reset ball position
            speedx = 12  # Reset speed
            speedy = 12
    ...
    if gameOver:
        img = cv2.addWeighted(imgGameOver, 1, img, 0, 0)
        cv2.putText(img, str(score[0] + score[1]).zfill(2), (870, 660), cv2.FONT_HERSHEY_COMPLEX, 5, (204, 255, 153), 8)  # Adjusted position
    else:
        # Increase speed periodically
        if time.time() - last_speed_increase_time > SPEED_INCREASE_INTERVAL:
            speedx = min(speedx * 1.2, MAX_SPEED)  # Smaller increment
            speedy = min(speedy * 1.2, MAX_SPEED)  # Smaller increment
            last_speed_increase_time = time.time()
    ...
    if key == ord('r'):
        ...
        # Clear the overlay to prevent overlapping scores
        img = cv2.addWeighted(imgBackground, 1, img, 0, 0)
    if key == ord('q'):
        break
