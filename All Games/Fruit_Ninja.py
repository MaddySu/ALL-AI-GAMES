...
GAME_DURATION = 2 * 60  # 15 minutes in seconds
...
for fruit in fruits:
    fruit_images[fruit] = load_and_resize_image(f'assest/Fruit Ninja/img/fruits/{fruit}.png', FRUIT_SIZE)
    cut_fruit_images[fruit] = load_and_resize_image(f'assest/Fruit Ninja/img/cut_fruits/cut_{fruit}.png', FRUIT_SIZE)

# Load and resize the image to overlay on the window
window_overlay_image = load_and_resize_image('assest/Fruit Ninja/img/BG.png', (1920, 1080))
finger_image = load_and_resize_image('assest/Fruit Ninja/img/Baile.png', (140, 140))  # Example size

# Initialize pygame for sound
pygame.mixer.init()
cut_sound = pygame.mixer.Sound('assest/Fruit Ninja/sounds/sound.mp3')
...
while True:
    ...
    key = cv2.waitKey(1)
    if key == ord('s') or key == ord('S') and not game_start:
        game_start = True
    ...
    if game_start and time.time() - start_time > GAME_DURATION:
        ...
        game_over_image = load_and_resize_image('assest/Fruit Ninja/img/GO.png', (SCREEN_WIDTH, SCREEN_HEIGHT))
        ...
        cv2.putText(frame, f"{score}", (900, 650), cv2.FONT_HERSHEY_SIMPLEX, 4, (0, 255, 255), 8)
